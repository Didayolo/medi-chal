...
class DataManager:
	...
	def __init__(self, basename="", input_dir="", verbose=False, replace_missing=True, filter_features=False, max_samples=float('inf')):
			...
			self.data['X_train'] = np.array(Xtr)
			self.data['Y_train'] = np.array(Ytr)
			self.data['X_valid'] = np.array(Xva)
			self.data['X_test'] = np.array(Xte)
	...
	def loadData(self, filename, verbose=True, replace_missing=True):
		...
		if self.info['format']=='dense' and replace_missing and np.any(map(np.nan, data)):#np.isnan(data).any():
			vprint (verbose, "Replace missing values by 0.")
	...
	def loadType(self, filename, verbose=True):
		...
		if os.path.isfile(filename):
			...
			type_list = [self.info['feat_type']] * self.info['feat_num']
	...
	def getInfo(self, filename, verbose=True):
		...
		if os.path.exists(filename):
			...
			vprint(verbose, "Info file found : " + 
				self.input_dir + '/' + self.basename + '_public.info')
		else:    
			vprint(verbose, "Info file NOT found : " + 
				self.input_dir + '/' + self.basename + '_public.info')            
			...
			self.getNbrPatterns(self.input_dir, self.basename, 'train')
			self.getNbrPatterns(self.input_dir, self.basename, 'valid')
			self.getNbrPatterns(self.input_dir, self.basename, 'test')
		...
		if 'feat_num' not in self.info.keys():
			...
			if self.info['format'] == 'dense':
				...
				for filename in filenames:
					...
					last_column_feature = [int(x.split(':')[0]) for x in last_column]
	...
	def getNbrPatterns(self, input_dir, basename, datatype):
		''' Get the number of patterns directly from the data file (in case we do not have an info file) '''
		line_num = data_converter.num_lines(os.path.join(input_dir, basename + '_' + datatype + '.data'))
