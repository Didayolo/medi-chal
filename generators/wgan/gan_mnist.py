...
OUTPUT_DIM = 1069  # Number of columns in the data
...
fixed_large_noise = tf.constant(
    np.random.normal(size=(10000, 100)).astype('float32'))
fixed_large_noise_samples = Generator(10000, noise=fixed_large_noise)
...
train_gen, dev_gen, test_gen = lib.mimic.load(BATCH_SIZE, BATCH_SIZE)
    ...
    for iteration in range(ITERS):
        ...
        if iteration % 1000 == 999:
            ...
            if iteration % 100000 == 99999:
                generate_rows(iteration + '_large', _data, small=False)
